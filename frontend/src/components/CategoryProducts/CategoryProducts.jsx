...
function CategoryProducts() {
  const { cat, priceRange, sortprice } = useSelector(
    (state) => state.categorySilice
  );
  const { id } = useParams();
  const { data, loading } = useFetch(
    `products?populate=*&[filters][categories][title]=${id}${cat
      .map((item) => `&[filters][sub_categories][id][$eq]=${item}`)
      .join("&")}&[filters][price][$lte]=${priceRange}&sort=price:${sortprice}`
  );
